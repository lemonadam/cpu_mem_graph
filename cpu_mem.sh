...
for ((j=1;j<=$device_num;j++));
    ...
    htmlname_mem=${device_serialname}_mem.html
    date=`date`
    echo $date
    sed -i '' '/\<\/body\>/i\  
    '"$date"'\
...
do
        ...
        if [ "$device_num" -eq 1 ]
            ...
            cpu=`awk 'BEGIN{RS=""}NF{$1=$1;print RS$0}' ${device_serialname}_cpu.log`
            # echo $cpu
            echo "myChart.setDataArray([" $cpu "], 'blue');" > ${device_serialname}_log_tmp.log
            cpu_tmp=`more ${device_serialname}_log_tmp.log`
            # echo $cpu_tmp
            # sed -i '' '/new/a\  
            # '"$cpu_tmp"'\
            # ' $htmlname 
            python merge.py $htmlname ${device_serialname}_log_tmp.log
            ...
            if [[ $mem_judge =~ 'No process' ]]; then
               ...
               echo "["$i","`adb -s ${device_serialname} shell dumpsys meminfo $1 | grep 'Native Heap' |grep -v ':' |awk '{print $8}'`"]," >> ${device_serialname}_memN.log &
               echo "["$i","`adb -s ${device_serialname} shell dumpsys meminfo $1| grep 'Dalvik Heap' |awk '{print $8}'`"]," >> ${device_serialname}_memD.log & 
            fi

            
            sleep $delay
            mem_N=`awk 'BEGIN{RS=""}NF{$1=$1;print RS$0}' ${device_serialname}_memN.log`
            mem_D=`awk 'BEGIN{RS=""}NF{$1=$1;print RS$0}' ${device_serialname}_memD.log`
            echo "myChart.setDataArray([" $mem_N "], 'blue');" > ${device_serialname}_log_memN_tmp.log
            echo "myChart.setDataArray([" $mem_D "], 'green');" > ${device_serialname}_log_memD_tmp.log
            memN_tmp=`more ${device_serialname}_log_memN_tmp.log`
            memD_tmp=`more ${device_serialname}_log_memD_tmp.log`
            python merge.py $htmlname_mem ${device_serialname}_log_memN_tmp.log
            python merge.py $htmlname_mem ${device_serialname}_log_memD_tmp.log
            # sed -i '' '/new/a\  
            # '"$memN_tmp"'\
            # ' $htmlname_mem
            # sed -i '' '/new/a\  
